...
int mlv_dump (int argc, char *argv[], int stripes)
{
    ...
    struct option long_options[] = {
        ...
        {"dig", no_argument, &didng_output, 1 },
        {"no-cs",  no_argument, &chroma_smooth_method,  0 },
        {"2x2",  no_argument, &chroma_smooth_method,  2 },
        {"3x3",  no_argument, &chroma_smooth_method,  3 },
        {"5x5",  no_argument, &chroma_smooth_method,  5 },
        {0,         0,                 0,  0 }
    };
    ...
    //set dng flags
    if(didng_output || chroma_smooth_method)
        dng_output = 1;
...
read_headers:
        ...
        if(!memcmp(buf.blockType, "MLVI", 4))
        {
            ...
            if(only_metadata_mode){
                int max_name_len = strlen(input_filename) + 16;
                char *metafilename = malloc(max_name_len);
                FILE *out_file = NULL;

                strncpy(metafilename, input_filename, max_name_len);

                strcpy(&metafilename[strlen(metafilename) - 3], "LOG");

                out_file = fopen(metafilename, "wb+");
                ...
                fprintf(out_file, "File Header (MLVI)\r\n");
                fprintf(out_file, "    Size        : 0x%08X\r\n", file_hdr.blockSize);
                fprintf(out_file, "    Ver         : %s\r\n", file_hdr.versionString);
                fprintf(out_file, "    GUID        : %08" PRIu64 "\r\n", file_hdr.fileGuid);
                fprintf(out_file, "    FPS         : %f\r\n", (double)file_hdr.sourceFpsNom / (double)file_hdr.sourceFpsDenom);
                fprintf(out_file, "    Frames Video: %d\r\n", file_hdr.videoFrameCount);
                fprintf(out_file, "    Frames Audio: %d\r\n\r\n", file_hdr.audioFrameCount);
        ...
        else
        {
            ...
            else if(!memcmp(buf.blockType, "LENS", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "Lens Information\r\n");
                    fprintf(out_file, "     Name:        '%s'\r\n", lens_info.lensName);
                    fprintf(out_file, "     Serial:      '%s'\r\n", lens_info.lensSerial);
                    fprintf(out_file, "     Focal Len:   %d mm\r\n", lens_info.focalLength);
                    fprintf(out_file, "     Focus Dist:  %d mm\r\n", lens_info.focalDist);
                    fprintf(out_file, "     Aperture:    f/%.2f\r\n", (double)lens_info.aperture / 100.0f);
                    fprintf(out_file, "     IS Mode:     %d\r\n", lens_info.stabilizerMode);
                    fprintf(out_file, "     AF Mode:     %d\r\n", lens_info.autofocusMode);
                    fprintf(out_file, "     Lens ID:     0x%08X\r\n", lens_info.lensID);
                    fprintf(out_file, "     Flags:       0x%08X\r\n\r\n", lens_info.flags);
            ...
            else if(!memcmp(buf.blockType, "STYL", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "Camera Styles\r\n");
                    fprintf(out_file, "     picStyle:   %d\r\n", block_hdr.picStyleId);
                    fprintf(out_file, "     contrast:   %d\r\n", block_hdr.contrast);
                    fprintf(out_file, "     sharpness:  %d\r\n", block_hdr.sharpness);
                    fprintf(out_file, "     saturation: %d\r\n", block_hdr.saturation);
                    fprintf(out_file, "     colortone:  %d\r\n\r\n", block_hdr.colortone);
            ...
            else if(!memcmp(buf.blockType, "WBAL", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "White Balance\r\n");
                    fprintf(out_file, "     Mode:   %d\r\n", wbal_info.wb_mode);
                    fprintf(out_file, "     Kelvin:   %d\r\n", wbal_info.kelvin);
                    fprintf(out_file, "     Gain R:   %d\r\n", wbal_info.wbgain_r);
                    fprintf(out_file, "     Gain G:   %d\r\n", wbal_info.wbgain_g);
                    fprintf(out_file, "     Gain B:   %d\r\n", wbal_info.wbgain_b);
                    fprintf(out_file, "     Shift GM:   %d\r\n", wbal_info.wbs_gm);
                    fprintf(out_file, "     Shift BA:   %d\r\n\r\n", wbal_info.wbs_ba);
            ...
            else if(!memcmp(buf.blockType, "IDNT", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "Camera Identification\r\n");
                    fprintf(out_file, "     Camera Name:   '%s'\r\n", idnt_info.cameraName);
                    fprintf(out_file, "     Camera Serial: '%s'\r\n", idnt_info.cameraSerial);
                    fprintf(out_file, "     Camera Model:  0x%08X\r\n\r\n", idnt_info.cameraModel);
            ...
            else if(!memcmp(buf.blockType, "RTCI", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "Date & Time\r\n");
                    fprintf(out_file, "     Date:        %02d.%02d.%04d\r\n", rtci_info.tm_mday, rtci_info.tm_mon + 1, 1900 + rtci_info.tm_year);
                    fprintf(out_file, "     Time:        %02d:%02d:%02d (GMT+%d)\r\n", rtci_info.tm_hour, rtci_info.tm_min, rtci_info.tm_sec, rtci_info.tm_gmtoff);
                    fprintf(out_file, "     Zone:        '%s'\r\n", rtci_info.tm_zone);
                    fprintf(out_file, "     Day of week: %d\r\n", rtci_info.tm_wday);
                    fprintf(out_file, "     Day of year: %d\r\n", rtci_info.tm_yday);
                    fprintf(out_file, "     Daylight s.: %d\r\n\r\n", rtci_info.tm_isdst);
            ...
            else if(!memcmp(buf.blockType, "EXPO", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "Exposure\r\n");
                    fprintf(out_file, "     ISO Mode:   %d\r\n", expo_info.isoMode);
                    fprintf(out_file, "     ISO:        %d\r\n", expo_info.isoValue);
                    fprintf(out_file, "     ISO Analog: %d\r\n", expo_info.isoAnalog);
                    fprintf(out_file, "     ISO DGain:  %d/1024 EV\r\n", expo_info.digitalGain);
                    fprintf(out_file, "     Shutter:    %" PRIu64 " µs (1/%.2f)\r\n\r\n", expo_info.shutterValue, 1000000.0f/(float)expo_info.shutterValue);
            ...
            else if(!memcmp(buf.blockType, "RAWI", 4))
            {
                ...
                if(only_metadata_mode){
                    ...
                    fprintf(out_file, "Resolution \r\n");
                    fprintf(out_file, "    Res:  %dx%d\r\n\r\n", block_hdr.xRes, block_hdr.yRes);
                    fprintf(out_file, "Raw Info\r\n");
                    fprintf(out_file, "      api_version      0x%08X\r\n", block_hdr.raw_info.api_version);
                    fprintf(out_file, "      height           %d\r\n", block_hdr.raw_info.height);
                    fprintf(out_file, "      width            %d\r\n", block_hdr.raw_info.width);
                    fprintf(out_file, "      pitch            %d\r\n", block_hdr.raw_info.pitch);
                    fprintf(out_file, "      frame_size       0x%08X\r\n", block_hdr.raw_info.frame_size);
                    fprintf(out_file, "      bits_per_pixel   %d\r\n", block_hdr.raw_info.bits_per_pixel);
                    fprintf(out_file, "      black_level      %d\r\n", block_hdr.raw_info.black_level);
                    fprintf(out_file, "      white_level      %d\r\n", block_hdr.raw_info.white_level);
                    fprintf(out_file, "      active_area.y1   %d\r\n", block_hdr.raw_info.active_area.y1);
                    fprintf(out_file, "      active_area.x1   %d\r\n", block_hdr.raw_info.active_area.x1);
                    fprintf(out_file, "      active_area.y2   %d\r\n", block_hdr.raw_info.active_area.y2);
                    fprintf(out_file, "      active_area.x2   %d\r\n", block_hdr.raw_info.active_area.x2);
                    fprintf(out_file, "      exposure_bias    %d, %d\r\n", block_hdr.raw_info.exposure_bias[0], block_hdr.raw_info.exposure_bias[1]);
                    fprintf(out_file, "      cfa_pattern      0x%08X\r\n", block_hdr.raw_info.cfa_pattern);
                    fprintf(out_file, "      calibration_ill  %d\r\n\r\n", block_hdr.raw_info.calibration_illuminant1);
