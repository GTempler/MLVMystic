...
int raw_get_pixel2(int x, int y) {
...
static void compute_black_noise(int x1, int x2, int y1, int y2, int dx, int dy, double* out_mean, double* out_stdev)
{
    ...
    for (y = y1; y < y2; y += dy)
    {
        for (x = x1; x < x2; x += dx)
        {
            double dif = raw_get_pixel2(x, y) - mean;
            stdev += dif * dif;
        }
    }
    stdev /= (num-1);
...
static int hdr_interpolate()
{
    ...
    */
    ...
    for (y = 2; y < h-2; y ++)
    {
        ...
        for (x = 0; x < w; x ++)
        {
            interp[x + y * w] = (raw_get_pixel2(x, y+2) + raw_get_pixel2(x, y-2)) / 2;
            native[x + y * w] = raw_get_pixel2(x, y);
        }
    }
