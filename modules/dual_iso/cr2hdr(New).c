    ...
    },
    {
        "Bad pixel handling", (struct cmd_option[]) {
          //{ &fix_pink_dots,  1, "--pink-dots",        "fix pink dots with a early chroma smoothing step" },
            { &cr_fix_bad_pixels, 1, "--bad-pix",          NULL },
            { &cr_fix_bad_pixels, 2, "--really-bad-pix",   "aggressive bad pixel fix, at the expense of detail and aliasing" },
            { &cr_fix_bad_pixels, 0, "--no-bad-pix",       "disable bad pixel fixing (try it if you shoot stars)" },
...
int raw_get_pixel_2(int x, int y) {
...
static void compute_black_noise(int x1, int x2, int y1, int y2, int dx, int dy, double* out_mean, double* out_stdev)
{
    ...
    for (y = y1; y < y2; y += dy)
    {
        for (x = x1; x < x2; x += dx)
        {
            double dif = raw_get_pixel_2(x, y) - mean;
            stdev += dif * dif;
        }
    }
    stdev /= (num-1);
...
static void find_and_cr_fix_bad_pixels(int dark_noise, int bright_noise, int* raw2ev, int* ev2raw)
{
    int w = raw_info.width;
    int h = raw_info.height;
    ...
    for (y = 6; y < h-6; y ++)
    {
        for (x = 6; x < w-6; x ++)
        {
            ...
            if (!BRIGHT_ROW || is_cold)
            {
                ...
                if (cr_fix_bad_pixels == 2)    /* aggressive */
                {
...
static int hdr_interpolate()
{
    ...
    */
    ...
    for (i = 0; i < 65536; i++)
    {
        ...
        double c2 = -cos(COERCE(ev2 - fullres_start, 0, fullres_transition)*3.14159265359/fullres_transition);
        double f = (c2+1) / 2;
        fullres_curve[i] = f;
    }
    ...
    if (cr_fix_bad_pixels)
    {
        /* best done before interpolation */
        find_and_cr_fix_bad_pixels(dark_noise, bright_noise, raw2ev, ev2raw);
    }
 ...
 //       amaze_demosaic_RT(rawData, red, green, blue, 0, 0, w, h);
        ...
        for (y = 5; y < h-5; y ++)
        {
            ...
            for (x = 5; x < w-5; x ++)
            {
                ...
                if (!BRIGHT_ROW)
                {
                    ...
                    if (fullres_curve[raw_get_pixel16(x, y)] > fullres_thr && !debug_edge)
                    {
                ...
                else if (raw_get_pixel16(x, y) < white_darkened && !debug_edge)
                {
    ...
    */
    ...
    for (i = 0; i < 65536; i++)
    {
        ...
        double c = -cos(MAX(MIN(ev-(max_ev-overlap),overlap),0)*3.14159265359/overlap);
        double k = (c+1) / 2;
        mix_curve[i] = k;
    }
